...
rl = pd.read_excel(r"Data\reliance_data.xlsx",  header=0)
...
def bollinger_bands():
    rl["Upper Band"] = rl[f"20_SMA"] + (2 * rl["SD"]) # upper band formula
    rl["Middle band"] =rl[f"20_SMA"] # middle band formula 
    rl["Lower Band"] = rl[f"20_SMA"] - (2 * rl["SD"]) # Lower band formula
...
def sma(x):
    rl[f"{x}_SMA"]= rl["Close"].rolling(window = x).mean() # 20 SMA formula 

    rl["price_vs_sma"]= (rl['Close'] - rl[f"{x}_SMA"])/ rl[f"{x}_SMA"] # sma deviation 
